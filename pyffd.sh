...
ENTITIES_OVERRIDE="/tmp/overrides"

# Directory where pyff pipeline is configured to download entities
DOWNLOADED_ENTITIES="${DATADIR}/downloads"
mkdir -p "${DOWNLOADED_ENTITIES}"
...
NGINX_ROOT_LINK="${DATADIR}/nginx-root"

# Blue-green deployment directories
DEPLOY_BLUE="${DATADIR}/deploy/blue"
DEPLOY_GREEN="${DATADIR}/deploy/green"
...
# Run pipelines
# ----------------------------------
${VENV}/bin/pyff --loglevel=${LOGLEVEL} /filter-sps.fd
${VENV}/bin/pyff --loglevel=${LOGLEVEL} /filter-idps.fd
